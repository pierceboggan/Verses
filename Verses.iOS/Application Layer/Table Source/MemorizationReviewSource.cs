...
{
	public class MemorizationReviewSource : UITableViewSource
	{
		...
		string REVIEW_BUTTON_CELL = "REVIEW_BUTTON_CELL";
		string SECTION_EMPTY_CELL = "SECTION_EMPTY_CELL";

		public MemorizationReviewSource (MemorizationTableViewController controller, ObservableSortedList<Verse> verses)
		{
			data = verses;
			...
			if (!sectionIsEmpty) {
				if (indexPath.Section == 0) {
					...
					if (indexPath.Row == 0) {
						...
						HandleReviewButtonCellTapped (tableView);
					}
				}
			}
		}
			...
			if (sectionIsEmpty) {
				...
				if (section == 0) {
					...
					if (indexPath.Row == 0) {
						...
						return GetReviewButtonCell (tableView, indexPath);
					}
				}
			}
		} 
		...
		void HandleReviewButtonCellTapped (UITableView tableView)
		{
			if (selectedVerses != null && selectedVerses.Count != 0)
			{
			...
			{
				new UIAlertView("No Selected Verses", "Whoops! Select the verses you wish to review first!", null, "Okay", null).Show();
			}
		}
	}
}